      ...
      {multi ? (
        <>
          <div className="flex items-center justify-between border-b border-neutral-200 px-4 py-3">
            <div className="text-sm font-bold">{selectedElementIds.length}個のレイヤーを選択中</div>
            <div className="flex gap-1">
              <IconBtn title="まとめて複製" onClick={() => duplicateElements(selectedElementIds)}>⧉</IconBtn>
              <IconBtn title="まとめて削除" danger onClick={() => deleteElements(selectedElementIds)}>🗑</IconBtn>
            </div>
          </div>
          <Section title="整列">
            <div className="flex flex-wrap gap-1">
              <AlignBtn label="左" onClick={() => alignSelected("x", "start")} />
              <AlignBtn label="左右中央" onClick={() => alignSelected("x", "center")} />
              <AlignBtn label="右" onClick={() => alignSelected("x", "end")} />
              <AlignBtn label="上" onClick={() => alignSelected("y", "start")} />
              <AlignBtn label="上下中央" onClick={() => alignSelected("y", "center")} />
              <AlignBtn label="下" onClick={() => alignSelected("y", "end")} />
            </div>
          </Section>
          <Section title="レイヤー">
            ...
            {slide.background.image?.startsWith("/api/assets/") && (
              <div className="pt-1">
                <button
                  ...
                  title="この背景を、動かせる画像レイヤー(オブジェクト単位)に分解します。編集したいページだけ実行してください(1〜2分)"
                ...
                >
                  {decomposing ? (
                    <>
                      <span className="cd-spin">✦</span> 分解中…
                    </>
                  ) : (
                    <>✦ レイヤーに分解</>
                  )}
                </button>
                <p className="mt-1 px-0.5 text-[10px] leading-snug text-neutral-400">
                  背景を動かせる画像に分解して編集可能に。編集したいページだけ実行(1〜2分)
                </p>
              </div>
            )}
          </Section>
