...
- 構成の意味付け: 課題→実物が散らばる/困っている場面 / 3ステップ→実物が3つ連なる / 効果→実物と上昇する形。**本文の項目数とイラストの構成単位の数を必ず揃える**(項目が4つなら4つの形が並ぶ/連なる構図)と、絵だけで論理構造が伝わる
- モチーフはテキスト用余白の反対側に描く
- 【紙面構造】本文ページ(takeawayがあるページ)では必ず指定する: 上端約170pxは見出し用にほぼ無地("plain clean band across the top")、下端約90pxは結論帯用にほぼ無地("plain clean strip across the bottom")。モチーフはその間の中段に収める
  ...
  "pages": [
    {
      ...
      "texts": [ { "role": "kicker|title|subtitle|body|stat|label|takeaway", "text": "実際の文言" } ]
    }
  ]
}

# 紙面の設計思想(最重要)
スライドは「読む資料」ではなく「一目で結論が伝わる紙面」にする。良い提案資料の共通構造:
- 【結論ファースト】表紙以外の全ページに role:"takeaway" を必ず1つ入れる。そのページで一番言いたい結論の一文(全角30文字以内)。紙面最下部の帯に大きく表示される。本文を読まなくてもtakeawayだけ追えば資料の論旨が通るように書く
- 【番号付き見出し】表紙以外のtitleは「1. 現状の課題」「2. 解決アプローチ」のように通し番号を付けた短い見出し(全角16文字以内)。ページ上部に固定表示される
- 【主張のサブタイトル】見出しの下に、そのページの主張を断定する一文をsubtitleで置く(例: title「避けるべきモデル」+ subtitle「高額講座一本勝負は、このターゲットと相性が悪い」)
- 【構造化された本文】bodyは文章ではなく短い項目にする。手順・流れは「① リスト追加」「② 無料掲載の提案」のような番号付き(2〜4個)、並列の要点は「・」始まり(2〜5個)。1項目は全角30文字以内。項目を読む順番=論理の流れになるように
- 1ページ1メッセージ。情報を詰め込みたくなったらページを分ける

# 入力からの変換規則
依頼文が散文・相談文・他資料の下書きでも、そのまま貼らずに次の規則で紙面に変換する:
- 【肉付け】入力が一行でも薄い資料にしない。一流のコンサルタントとして、現状分析・具体的な打ち手・商品/施策の設計・手順・効果見込みまで自分で考えて「実のある提案」に仕上げる。一般論の羅列で埋めない
- 【具体は絶対に残す】入力にある数値・価格・件数・期間・固有名詞(例:「20万円」「30本」「1年間」「月額1,480円」)は要約で潰さず、そのまま body/stat の主役にする。提案に必要な数値が入力に無ければ「20〜30万円(仮)」のように仮であることを明記して具体的に置く。数字のない提案は弱い
- 【説明は捨てる】背景説明・推測・ためらい(「〜と思います」「〜の可能性がある」)は削除し、断定の短文に変換する
- 【外向けの完成品】スライドは読者にそのまま見せる成果物。調査・作成過程の痕跡を本文に書かない: 「公式ページでは〜と記載されています」「Web調査によると」「〜とのこと」のような出典言及・伝聞調は禁止。事実は資料の言葉として断定する(例:「来社客への貸出、駅・街中での返却にも対応」)。出典はスライドの外で管理される
- 【項目の圧縮】bodyの各項目は「短いラベル+具体」に圧縮する(入力「オリジナル番傘を30本制作して1年間運用する」→「・制作30本 / 運用1年間」)。入力の一文をそのまま写さない
- 【結論の合成】takeawayは入力の太字主張をそのまま使うか、そのページの項目群から「つまり何が言いたいか」を新しく一文に合成する
- 【論理の抽出】入力から論理の流れ(現状→課題の構造→原因→強み→転換→商品設計→効果→次アクション など)を抽出して1ページ1主張に再構成する。入力にページ構成が書かれていればそれに従う
- 【デザインは論理の視覚化】ページの論理タイプに合わせてmotifと構図を決める: 流れ・手順→連なる構図 / 比較・転換→左右の対比 / 強調・商品→1つの大きな実物 / 全体像→中心+周辺の構成。装飾のための装飾はしない

# motif / space
- motif: ページの「伝える内容」から比喩を起こす(課題→散らばる断片、3ステップ→3つの連結形、など)。imagePromptはこのmotifを必ず描く
- space: 本文(body一式)を置く余白の位置。motifはその反対側に置く。ページ間で適度に変化させ、同じ側を3ページ以上続けない。テキストが多いページはleft/rightの縦長余白が安全
...
# texts
- 各ページ: kicker(短い英字ラベル) + title + body 2〜5個 + takeaway。必要なら stat(短い数値)や label(注釈)を足す(合計3〜8個)
- 表紙(1ページ目)だけは takeaway なしで、title + subtitle 中心の大きな構成にする
...
export async function generateImage2Slide(
  ...
  const texts = (page.texts ?? []).slice(0, 9);
  // 構造化紙面(参考デッキの共通構造): takeaway(結論帯)があるページは
  // 「上部=番号付き見出し / 中段=本文+モチーフ / 最下部=結論の帯」に固定する。
  // 表紙などtakeawayのないページは従来どおり1ゾーンの自由配置。
  const takeaway = texts.find((t) => t.role === "takeaway");
  const headerTexts = takeaway
    ? [...texts.filter((t) => t.role === "kicker"), ...texts.filter((t) => t.role === "title")]
    : [];
  const bodyTexts = takeaway
    ? texts.filter((t) => t !== takeaway && !headerTexts.includes(t))
    : texts;
  const headerPlacements = typesetHeader(headerTexts);
  const headerBottom = headerPlacements.length
    ? Math.max(...headerPlacements.map((p) => p.y + p.h))
    : 140;
  ...
  try {
          ...
          [
            ...
            },
            {
              ...
              text:
                ...
                bodyTexts.map((t, j) => `${j}: [${t.role}] ${t.text}`).join("\n") +
                (page.space ? `\n制作意図ではテキスト用余白は${page.space}側の想定。実際の画像を見て判断してください。` : "") +
                (takeaway
                  ? `\nこのページは上部(y<${bodyTop})に見出し、最下部(y>${BAND.y - 16})に結論帯を別途配置するため、zoneはその間から選ぶこと。`
                  : ""),
            },
          ],
          8000,
    ...
    let layoutTexts = texts;
    let placements: Placement[];
    if (takeaway) {
      // 見出しは上部固定、本文はゾーン(中段に収める)、結論は最下部の帯
      const bodyZone = clampBodyZone(zone ?? zoneForSpace(page.space), bodyTop, BAND.y - 20);
      layoutTexts = [...headerTexts, ...bodyTexts];
      placements = [
        ...headerPlacements,
        ...typesetZone(bodyTexts, bodyZone).map((p) => ({ ...p, index: p.index + headerTexts.length })),
      ];
      // kickerはブランド色を初期値に(コントラスト不足なら実測で置き換わる)
      for (const p of headerPlacements) {
        if (headerTexts[p.index]?.role === "kicker") p.colorHex = theme.colors.brand;
      }
    } else {
      placements = typesetZone(texts, zone ?? zoneForSpace(page.space));
    }
    let elements: SlideElement[] = await applyContrast(placements, layoutTexts, cropped, theme);
    if (takeaway) elements = [...elements, ...takeawayElements(takeaway.text)];
...
// ---- 構造化紙面(見出し帯・結論帯) --------------------------------------
// 参考にした提案資料の共通構造を決定的に再現する:
//   上部   = kicker + 番号付き見出し(左揃え・固定位置)
//   中段   = 本文(ビジョンが見つけた余白ゾーン。中段に収まるようクランプ)
//   最下部 = 結論の帯(角丸の帯 + 白文字でそのページの一番言いたいこと)

export const BAND = { x: 64, y: 636, w: 1152, h: 56, r: 28 };

// 見出し(kicker/title)を上部に固定で組む。index は渡した配列内の位置
export function typesetHeader(texts: PlanText[]): Placement[] {
  const out: Placement[] = [];
  let y = 38;
  texts.forEach((t, i) => {
    if (t.role === "kicker") {
      out.push({ index: i, x: 64, y, w: 700, h: 22, fontSizePx: 14, fontWeight: 700, align: "left", colorHex: "", lh: 1.4 });
      y += 32;
    } else {
      const w = 1100;
      const steps = [42, 38, 34, 30];
      const fits = steps.filter((s) => linesFor(t.text, s, w) <= 2);
      const size = fits.find((s) => !hasOrphanLine(t.text, s, w)) ?? fits[0] ?? 30;
      const h = Math.ceil(linesFor(t.text, size, w) * size * 1.25) + 6;
      out.push({ index: i, x: 64, y, w, h, fontSizePx: size, fontWeight: 900, align: "left", colorHex: "", lh: 1.25 });
      y += h + 10;
    }
  });
  return out;
...
// 本文ゾーンを「見出しの下〜結論帯の上」の中段に収める。
// クランプの結果つぶれてしまう場合は中段いっぱいの安全なゾーンに倒す
export function clampBodyZone(z: Zone, top: number, bottom: number): Zone {
  const y = clamp(z.y, top, Math.max(top, bottom - 200));
  const h = Math.min(z.y + z.h, bottom) - y;
  if (h < 180 || bottom - top < 180) return { x: 64, y: top, w: SLIDE_W - 128, h: Math.max(bottom - top, 180) };
  return { x: z.x, y, w: z.w, h };
}

// 結論の帯。帯はテーマトークン参照なのでテーマ変更に追従し、
// 帯もテキストも通常要素なのでエディタで動かせる
export function takeawayElements(text: string): SlideElement[] {
  const size = clamp(Math.floor((BAND.w - 96) / Math.max(textEm(text), 1)), 13, 20);
  const th = Math.ceil(size * 1.4) + 4;
  const band: ShapeEl = {
    id: uid(),
    type: "shape",
    shape: "rect",
  ...
  const label: TextEl = {
    ...
    text,
    x: BAND.x + 48,
    y: BAND.y + Math.round((BAND.h - th) / 2),
    w: BAND.w - 96,
    h: th,
    fontSize: size,
    fontWeight: 700,
...
function specFor(t: PlanText, zoneW: number): RoleSpec {
  switch (t.role) {
    ...
    case "takeaway":
      // 通常は帯として別組みされる。1ゾーン配置に混ざった時の保険
      return { size: 22, weight: 700, lh: 1.5, gap: 16 };
    default: // body
...
function roleRank(role: string): number {
  ...
  if (role === "label") return 4; // 注釈は後ろ
  if (role === "takeaway") return 5; // 結論は最後
