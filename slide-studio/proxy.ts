...
// 簡易トークン認証。SLIDE_STUDIO_API_TOKEN を設定した環境(社内Mac mini等)でのみ有効。
// 未設定ならすべて素通し(ローカル開発)。
//  - APIクライアント(AIka等): Authorization: Bearer <token>
...
export function proxy(req: NextRequest) {
  ...
  if (req.cookies.get("studio_token")?.value === token) return NextResponse.next();
