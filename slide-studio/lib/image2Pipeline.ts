...
- 【内容連動】そのページが伝える内容を視覚的に表すモチーフを必ず1つ決めて描く。例: 課題→散らばる・絡まる断片 / 解決・統合→ひとつに集まる形 / 3ステップ→3つの連結された形 / 成長・効果→上昇する形 / 比較→対置される2つの形 / 料金・選択→1つの際立つ形 / まとめ→収束する形。箇条書きの個数と図形の数を揃えると内容との対応が伝わる
- モチーフは抽象的なグラフィック(図形・イラスト的シェイプ)として、テキスト用余白の反対側に描く
