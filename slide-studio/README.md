...
2. ~~**画像レイヤー**~~ — **実装済み**。「✦画像」で挿絵・アイコン等を透過PNGとして単体生成(`/api/generate/image`)、Inspectorの「✦ 背景を透過」で既存画像から被写体を切り抜き(`/api/edit-image`、gpt-imageのedits)。いずれもgpt-image-2が透過非対応のためgpt-image-1系へ自動フォールバック。生成済み背景を複数パーツに分解する本格的なセグメンテーションは将来課題
