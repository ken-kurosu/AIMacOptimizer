...
- **PDF取り込み** — スライドPDFを「文字を消した背景画像 + 編集可能なテキスト要素」へ分解して取り込む。ページごとに自動で2経路: テキストレイヤーのあるPDF(PowerPoint/Keynote/Googleスライド書き出し等)は**文字・位置・サイズ・フォント感を正確に保ったまま打ち直せる**(APIキー不要・無料)。画像のみのPDF(NotebookLM等)はAIで文字を読み取って再配置。背景は「✦ レイヤーに分解」で画像編集も可能
...
AIka用のスキル実装とMac miniへのデプロイ手順は [docs/aika-skill.md](docs/aika-skill.md) を参照。

### 認証

`SLIDE_STUDIO_API_TOKEN` を設定すると全ページ・APIがトークン保護されます(未設定なら認証なし=ローカル用)。
