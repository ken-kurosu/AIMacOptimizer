...
- **AI一括生成** — 入力は「内容(自由記述)・ページ数・参考画像(任意)」の3つだけ。誰向けか・トーン・必須の数値などは内容に書けばすべて反映される。アートディレクション(配色9トークン+フォントペア)込みでデッキを生成
...
デッキを生成して保存し、編集画面のURLを返すAPIがあります。AIka等のエージェントからの利用を想定:
